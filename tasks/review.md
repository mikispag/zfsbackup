...
| Medium | Placeholder-only filesystems issued empty sync requests and unnecessary incremental-state queries. | Skip empty requests and sync bookmarks directly when there are no snapshots to send. CI exposed this on unsnapshotted parents; regression tests cover empty, unrelated and matching bookmarks. |
